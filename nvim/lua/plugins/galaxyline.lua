    ...
    LeftStatusMixedIndentWhiteSpace = {
      provider = function()
        ...
        if vim.fn.search([[\v(^\t+)]], 'nw') ~= 0 and vim.fn.search([[\v(^ +)]], 'nw') ~= 0 then
          cmd[[highlight link GalaxyLeftStatusMixedIndentWhiteSpace GalaxyMapperCommon6]]
          cmd[[match ErrorMsg /\t/]]
          return ' '
        end
        ...
        if vim.fn.search([[\v(^\t+)]], 'nw') ~= 0 then
          cmd[[highlight link GalaxyLeftStatusMixedIndentWhiteSpace GalaxyMapperCommon8]]
        else
          cmd[[highlight link GalaxyLeftStatusMixedIndentWhiteSpace GalaxyMapperCommon6]]
        end
    ...
    LeftGitDiffAdd = {
      ...
      provider = function()
        if require('galaxyline.provider_vcs').diff_add() then
          cmd[[highlight link GalaxyLeftGitDiffAdd GalaxyLeftGitDiffAddActive]]
          return '+' .. require('galaxyline.provider_vcs').diff_add()
        else
          cmd[[highlight link GalaxyLeftGitDiffAdd GalaxyLeftGitDiffInactive]]
          return '+0 '
        end
      end
    }
    ...
    LeftGitDiffModified= {
      ...
      provider = function()
        if require('galaxyline.provider_vcs').diff_modified() then
          cmd[[highlight link GalaxyLeftGitDiffModified GalaxyLeftGitDiffModifiedActive]]
          return '~' .. require('galaxyline.provider_vcs').diff_modified()
        else
          cmd[[highlight link GalaxyLeftGitDiffModified GalaxyLeftGitDiffInactive]]
          return '~0 '
        end
      end
    }
    ...
    LeftGitDiffRemove = {
      ...
      provider = function()
        if require('galaxyline.provider_vcs').diff_remove() then
          cmd[[highlight link GalaxyLeftGitDiffRemove GalaxyLeftGitDiffRemoveActive]]
          return '-' .. require('galaxyline.provider_vcs').diff_remove()
        else
          cmd[[highlight link GalaxyLeftGitDiffRemove GalaxyLeftGitDiffInactive]]
          return '-0 '
        end
      end
    }
    ...
    MidFileStatus = {
      provider = function()
        if vim.bo.modified then
          cmd[[highlight link GalaxyMidFileStatus GalaxyMidFileStatusModified]]
        elseif not vim.bo.modifiable then
          cmd[[highlight link GalaxyMidFileStatus GalaxyMidFileStatusRestricted]]
        elseif vim.bo.readonly then
          cmd[[highlight link GalaxyMidFileStatus GalaxyMidFileStatusReadonly]]
        elseif not vim.bo.modified then
          cmd[[highlight link GalaxyMidFileStatus GalaxyMidFileStatusUnmodified]]
        end
    ...
    RightLspError = {
      provider = function()
        ...
        if vim.lsp.diagnostic.get_count(0, 'Error') == 0 then
          cmd[[highlight link GalaxyRightLspError GalaxyLeftLspInactive]]
        else
          cmd[[highlight link GalaxyRightLspError GalaxyRightLspErrorActive]]
        end
    ...
    RightLspWarning = {
      provider = function()
        ...
        if vim.lsp.diagnostic.get_count(0, 'Warning') == 0 then
          cmd[[highlight link GalaxyRightLspWarning GalaxyLeftLspInactive]]
        else
          cmd[[highlight link GalaxyRightLspWarning GalaxyRightLspWarningActive]]
        end
    ...
    RightLspInformation = {
      provider = function()
        ...
        if vim.lsp.diagnostic.get_count(0, 'Information') == 0 then
          cmd[[highlight link GalaxyRightLspInformation GalaxyLeftLspInactive]]
        else
          cmd[[highlight link GalaxyRightLspInformation GalaxyRightLspInformationActive]]
        end
    ...
    RightLspHint = {
      provider = function()
        ...
        if vim.lsp.diagnostic.get_count(0, 'Hint') == 0 then
          cmd[[highlight link GalaxyRightLspHint GalaxyLeftLspInactive]]
        else
          cmd[[highlight link GalaxyRightLspHint GalaxyRightLspHintActive]]
        end
    ...
    RightFileType = {
      provider = function()
        if vim.bo.fileencoding == 'utf-8' then
          cmd[[highlight link GalaxyRightFileType GalaxyMapperCommon4]]
        else
          cmd[[highlight link GalaxyRightFileType GalaxyMapperCommon8]]
        end
...
local M = {}
M.self_color_gruvbox_dark = function()
